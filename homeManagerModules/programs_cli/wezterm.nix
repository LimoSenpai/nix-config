...
let
  hasStylixLib = config.lib ? stylix;
  stylixColors = if hasStylixLib then config.lib.stylix.colors.withHashtag else null;
  fallbackColors = {
    base00 = "#1a1b26";
    base01 = "#16161e";
    base02 = "#2f3549";
  ...
  stylixFonts = if hasStylix && (config.stylix.fonts ? monospace) then config.stylix.fonts.monospace else null;
  fontFamily = if stylixFonts != null then stylixFonts.name else "Maple Mono";
  terminalOpacity =
    if hasStylix && (config.stylix.opacity ? terminal)
    then config.stylix.opacity.terminal
    else 0.95;
in {
  options = {
